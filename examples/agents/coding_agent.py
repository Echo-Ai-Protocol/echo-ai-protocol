#!/usr/bin/env python3
"""Fixture-based CodingAgent for seeding ECHO request/EO/trace objects."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Seed coding EO/trace objects into ECHO")
    parser.add_argument("--base-url", default="http://127.0.0.1:8080")
    parser.add_argument("--integration-id", default="ext-ai-seed")
    parser.add_argument("--agent-name", default="CodingAgent")
    parser.add_argument("--skip-gate", action="store_true")
    parser.add_argument("--skip-signature", action="store_true")
    parser.add_argument("--run-tag", default="")
    parser.add_argument(
        "--tasks-file",
        default=str(base / "sample_tasks" / "coding_tasks.json"),
    )
    parser.add_argument(
        "--output",
        default="",
...
def _request_payload(agent_did: str, integration_id: str, run_token: str, task: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload["rq_id"] = make_id("request", integration_id, "coding", str(task["task_id"]), run_token)
    payload["agent_did"] = agent_did
    payload["request_embedding"] = f"coding_request::{task['problem']}"
    payload["constraints_embedding"] = f"coding_constraints::{task['constraints']}"
    payload["desired_output_type"] = "IMPLEMENTATION_HINTS"
...
def _eo_payload(integration_id: str, run_token: str, task: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload["eo_id"] = make_id("eo", integration_id, "coding", str(task["task_id"]), run_token)
    payload["problem_embedding"] = f"problem::{task['problem']}"
    payload["constraints_embedding"] = f"constraints::{task['constraints']}"
    payload["solution_embedding"] = f"solution::{task['solution_summary']}"
    ...
    payload["confidence_score"] = float(task.get("confidence_score", 0.85))
    payload["created_at"] = utc_now()
    return payload
...
def _trace_payload(
    ...
    payload["trace_id"] = make_id("trace", integration_id, "coding", str(task["task_id"]), run_token)
    payload["agent_did"] = agent_did
    payload["domain_embedding"] = f"domain::coding::{task['task_id']}"
...
def main() -> int:
    ...
    out_path = (
        ...
        if args.output
        else (default_output_dir() / f"coding_agent_{stable_slug(args.integration_id)}.json")
    )
    report: Dict[str, Any] = {
        "agent": "CodingAgent",
